...
export const DetailTabForIdea: FunctionComponent<DetailTabForIdeaProps> = (props: DetailTabForIdeaProps) => {
  ...
  const showDeleteModal = (item) => {
    context.showModal({
      type: "Question",
      title: "Fikir Sil",
      message: `${item.Name} isimli fikri listeden silmek istediğinize emin misiniz?`,
      onClick: async () => {
  ...
  const addItemToDynamicList = async () => {
    ...
    if (_result.succeeded === true) {
      context.showModal({
        type: "Success",
        title: "Fikir(ler) başarıyla eklendi",
        onClose: () => {
          context.hideModal();
        ...
        body=
        {
          <div className="">
            <div className="flex gap-3 justify-end items-center text-sm my-4">
              <div className="text-gray-700"><span className="text-gray-900 font-medium">{selectedItemIdList.length + selectedTempItemIdList.length} / 200</span> Fikir Seçildi</div>
              <Button isLoading={processLoading} buttonMd text="Seçili Fikirleri Ekle" design="button-blue-400 w-60" onClick={() => {
