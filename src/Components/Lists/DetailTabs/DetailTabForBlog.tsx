...
import { DynamicListBlogModel } from "../../../Models";
...
export interface DetailTabForBlogProps {
  DynamicListId: number
}

export const DetailTabForBlog: FunctionComponent<DetailTabForBlogProps> = (props: DetailTabForBlogProps) => {
  ...
  const getDynamicListItems = async (order: number, searchText: string, page: number, take: number) => {
    ...
    if (_result.succeeded === true) {
      let _tempArray: DynamicListBlogModel[] = []
      let _selectedItemIdList: number[] = []
      _result.data.Data.map((item) => (
        _tempArray.push(item.Blog),
        _selectedItemIdList.push(item.Blog.BlogId)
      ))
  ...
  const showDeleteModal = (item) => {
    context.showModal({
      type: "Question",
      title: "Fikir Sil",
      message: `${item.Title} isimli blog listeden silmek istediğinize emin misiniz?`,
      onClick: async () => {
  ...
  const addItemToDynamicList = async () => {
    ...
    if (_result.succeeded === true) {
      context.showModal({
        type: "Success",
        title: "Blog(lar) başarıyla eklendi",
        onClose: () => {
          context.hideModal();
  ...
  const returnBody = (e) => {
    return <><div className="lg:col-span-2 flex lg:block items-center flex items-center">
      ...
      <Image src={e.PhotoUrl} className="min-w-14 max-w-14 max-h-14 min-h-14 mr-2 object-contain  " />
    </div>
  ...
  const handleAddTempItem = (blog) => {

    if (selectedTempItemIdList.find(i => i === blog.Id)) {
      let _currentArray = selectedTempItemIdList.filter(i => i !== blog.Id);
      setSelectedTempItemIdList(_currentArray);
    }
    else {
      setSelectedTempItemIdList([...selectedTempItemIdList, blog.Id])
    }
  ...
  return (
    <>
      <Table
        ...
        addNewButton={
          <Button buttonMd textTiny color="text-blue-400" className="w-72" design="button-blue-100" text="Yeni Blog Ekle" hasIcon icon={<PlusIcon className="icon-sm mr-2" />} onClick={() => { setShowAddingModal(true) }} />}
        header={returnHeader()}
        renderItem={(e, i) => {
          return <div key={"list" + i} className="lg:grid-cols-11 px-2 border-b lg:h-20 border-gray-200 grid gap-4 items-center py-3 lg:py-0">
            ...
            <div className="lg:col-span-1 flex justify-between items-center">
              ...
              <div className="text-gray-700 flex gap-2 items-center">
                ...
                <ChevronRightIcon className="icon-md text-gray-700 border-l" onClick={() => { history.push(`/blog-duzenle/${e.BlogId}`) }} />
              </div>
            </div>
          </div>
        }}
        ...
        body=
        {
          <div className="">
            <div className="flex gap-3 justify-end items-center text-sm my-4">
              <div className="text-gray-700"><span className="text-gray-900 font-medium">{selectedItemIdList.length + selectedTempItemIdList.length} / 200</span> Blog Seçildi</div>
              <Button isLoading={processLoading} buttonMd text="Seçili Blogları Ekle" design="button-blue-400 w-60" onClick={() => {
