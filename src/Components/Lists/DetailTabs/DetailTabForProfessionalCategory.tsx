...
import { DynamicListProfessionalCategoryModel } from "../../../Models";
...
export interface DetailTabForPofessionalCategoryProps {
  DynamicListId: number
}

export const DetailTabForPofessionalCategory: FunctionComponent<DetailTabForPofessionalCategoryProps> = (props: DetailTabForPofessionalCategoryProps) => {
  ...
  const getDynamicListItems = async (order: number, searchText: string, page: number, take: number) => {
    ...
    if (_result.succeeded === true) {
      let _tempArray: DynamicListProfessionalCategoryModel[] = []
      let _selectedItemIdList: number[] = []

      _result.data.Data.map((item) => (
        _tempArray.push(item.ProfessionalCategory),
        _selectedItemIdList.push(item.ProfessionalCategory.CategoryId)
  ...
  const showDeleteModal = (item) => {
    context.showModal({
      type: "Question",
      title: "Kategori Sil",
      message: `${item.CategoryName} isimli profesyonel kategorisini listeden silmek istediğinize emin misiniz?`,
      onClick: async () => {
        ...
        renderItem={(e, i) => {
          return <div key={"list" + i} className="lg:grid-cols-7 px-2 border-b lg:h-20 border-gray-200 grid gap-4 items-center py-3 lg:py-0">
            <div className="lg:col-span-1 flex lg:block items-center">
              ...
              <Image src={e.CategoryPhoto} alt={e.CategoryName} className="min-w-14 max-w-14 max-h-14 min-h-14 mr-2 object-contain" />
            </div>
            ...
            <div className="lg:col-span-1 flex justify-between items-center">
              ...
              <div className="text-gray-700 flex gap-2 items-center">
                ...
                <ChevronRightIcon className="icon-md text-gray-700 border-l" onClick={() => { history.push(`/pro-kategori-detay/${e.CategoryId}`) }} />
              </div>
            </div>
          </div>
        }}
            ...
            <Table
              ...
              renderItem={(e, i) => {
                return <div key={"list" + i} className="lg:grid-cols-7 px-2 border-b lg:h-20 border-gray-200 grid gap-4 items-center py-3 lg:py-0">
                  <div className="lg:col-span-1 flex lg:block items-center">
                    <span className="p-sm-gray-700 lg:hidden mr-2">Kategori Görseli</span>
                    <Image src={e.PhotoPath} alt={e.CategoryName} className="min-w-14 max-w-14 max-h-14 min-h-14 mr-2 object-contain" />
                  </div>
                  <div className="lg:col-span-2 flex lg:block items-center flex items-center">
                    <span className="p-sm-gray-700 lg:hidden mr-2">Kategori Adı:</span>
                    <p className="text-black-700 text-sm">
