...
import { CheckIcon, ChevronRightIcon, CloseIcon, LikeButton, OrderCancelIcon, PacketUpIcon, SmileIcon, StarIcon } from "../Components/Icons";
...
export const StoreEvaluation: FunctionComponent<EvaluationListProps> = (props: EvaluationListProps) => {
  ...
  const [storeRate, setStoreRate] = useState<number>(0);
  ...
  const getStoreEvaluationsForDetail = async (order: number, searchText: string, page: number, take: number) => {
    setCardLoading(true);

    const _result = await ApiService.getStoreEvaluationsForDetail(props.id, page, take, searchText, order);

    if (_result.succeeded === true) {
      setStoreRate(_result.data.StoreRate);
  ...
  return (
    <>
      <div className="grid lg:grid-cols-4 gap-4 mt-4 border-b pb-4">
        <RateCard loading={cardLoading} bgColor="bg-gray-100" onStore label="Genel Mağaza Puanı" rateValue={storeRate} icon={<SmileIcon className="w-10 h-10 mr-2 text-blue-400 p-2 bg-white rounded-full" />} />
      ...
      <Table
        ...
        getDataFunction={getStoreEvaluationsForDetail}
