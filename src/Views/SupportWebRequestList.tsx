...
export const SupportWebRequestList: FunctionComponent = () => {
  ...
  const getSupportRequestList = async (order: number, searchText: string, page: number, take: number) => {
    ...
    const _result = await ApiService.getSupportRequestList(page, take, searchText, order, 1);
  ...
  const handleShowSupportDeleteModal = (item) => {
    context.showModal({
      ...
      message: `${item.Name} isimli kullanıcıya ait destek talebini silmek istediğinize emin misiniz?`,
      onClick: async () => {
  ...
  return (
    <div className="content-wrapper">
      <div className="portlet-wrapper">
        <h2 className="mb-5">Web Destek Talepleri</h2>
        <Table
          ref={tableEl}
