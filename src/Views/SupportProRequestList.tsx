...
export const SupportProRequestList: FunctionComponent = () => {
  ...
  const getSupportRequestList = async (order: number, searchText: string, page: number, take: number) => {
    ...
    const _result = await ApiService.getSupportRequestList(page, take, searchText, order, 2);
  ...
  const handleShowSupportDeleteModal = (item) => {
    context.showModal({
      ...
      message: `${item.Name} isimli profesyonele ait destek talebini silmek istediğinize emin misiniz?`,
      onClick: async () => {
  ...
  return (
    <div className="content-wrapper">
      <div className="portlet-wrapper">
        <h2 className="mb-5">Profesyonel Destek Talepleri</h2>
        <Table
          ref={tableEl}
          ...
          renderItem={(e, i) => {
            return <div key={"list" + i} className="lg:grid-cols-11 px-2 border-b min-h-20 py-5 border-gray-200 hidden lg:grid flex gap-4 items-center">
              ...
              <div className="lg:col-span-2">
                <Link to={{ pathname: `${"/pro-profesyonel-detay/" + e.StoreId}`, state: { prevTitle: "Profesyonel Destek Talepleri", prevPath: window.location.pathname, tabId: 1 } }} >
