...
import { CategoryPropertiesListModel, CategoryPropertyValueListModel, CategoryVariationsListModel, CategoryVariationValueListModel, ProductAdvertAdminDraftDetailModel, ProductImageModel } from "../../Models";
...
export interface ProductDetailPropModel {
  ...
  advertVariations: CategoryVariationValueListModel[],
  advertFeatures: CategoryPropertyValueListModel[],
            ...
            <h4 className="mb-4">Ürün Bilgileri</h4>
            {
              props.loading ?
                ...
                <div className="flex mb-2">
                  ...
                  <div className="text-sm text-gray-900 ml-2">{props.advertDetail?.Category}</div>
                </div>
            }
            {
              props.loading ?
                ...
                <div className="flex mb-2">
                  ...
                  <div className="text-sm text-gray-900 ml-2">{props.advertDetail?.ProductName}</div>
                </div>
            }
            {
              props.loading ?
                ...
                <div className="flex mb-2">
                  ...
                  <div className="text-sm text-gray-900 ml-2">{props.advertDetail?.Brand}</div>
                </div>
            }
            {
              props.loading ?
                ...
                <div className="flex mb-2">
                  ...
                  <div className="text-sm text-gray-900 ml-2">{props.advertDetail?.ModelNo}</div>
                </div>
            }
            {
              props.loading ?
                ...
                <div className="flex mb-2">
                  ...
                  <div className="text-sm text-gray-900 ml-2">{props.advertDetail?.ShortDescription ?? "-"}</div>
                </div>
            }
            {
              props.loading ?
                ...
                :
                <div className="flex mb-2">
                  <div className="min-w-1/3 flex p-sm-gray-700 items-center"><span>Ürün Durumu</span><span className="ml-auto">:</span></div>
                  {(props.advertDetail?.IsProductEnabled === true) ?
                    <div className="text-sm text-green-400 ml-2 font-medium">Aktif</div>
                    :
                    <div className="text-sm text-red-400 ml-2 font-medium">Pasif</div>
                  }
                </div>
            }
          </div>
          <div className="lg:w-1/2">
            <h4 className="mb-4">Ürün varyasyon Seçimleri</h4>
            {
              props.loading ?
                <Loading textMd />
                :
                props.advertVariations.map((item, index) => {
          ...
          <div className="lg:w-1/2">
            <h4 className="mb-4">Ürün Özellikleri</h4>
            {
              props.loading ?
                <Loading textMd />
                :
                props.advertFeatures.map((item, index) => {//DATE_RANGE
                  if (item.DataType === 1) {
                    return (
        ...
        <div className="flex w-full gap-8">
          {
            props.loading ?
              <Loading inputMd />
              :
              <div dangerouslySetInnerHTML={{ __html: props.advertDetail?.Description ?? "" }} className="ck-editor-links text-sm h-48 w-1/2 p-4 overflow-y-auto custom-scrollbar border border-gray-300 rounded-lg" >
              </div>
          }
        </div>
      </div>
