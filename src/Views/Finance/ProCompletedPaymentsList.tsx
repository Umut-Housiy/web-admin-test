...
export const ProCompletedPaymentsList: FunctionComponent = () => {
  ...
  const sortOptions = [
    { key: "2", value: "Ödeme Tarihine Göre Azalan" },
    { key: "1", value: "Ödeme Tarihine Göre Artan" }
  ];

  const getProCompletedPaymentsList = async (order: number, searchText: string, page: number, take: number) => {

    const _result = await ApiService.getProCompletedPaymentsList(page, take, searchText, order);
  ...
  return (
    <div className="content-wrapper">
      <div className="portlet-wrapper">
        <h2 className="mb-5">
          Tamamlanan Profesyonel Tahsilatları
        </h2>
        <Table
          ref={tableEl}
          emptyListText={"Tahsilat Bulunamadı"}
          getDataFunction={getProCompletedPaymentsList}
          ...
          renderItem={(e, i) => {
            return <div key={"list" + i} className="lg:grid-cols-11 px-2 border-b lg:h-20 border-gray-200 grid gap-4 items-center py-3 lg:py-0">
              ...
              <div className="lg:col-span-2 flex lg:block items-center">
                <span className="p-sm-gray-700 lg:hidden mr-2">Son Ödeme Tarihi:</span>
                {e.LastPayDateJSTime > 0 ?
                  <DateView className="text-sm text-red-400 mb-1" dateNumber={e.LastPayDateJSTime} pattern="dd/MM/yyyy" />
                  :
                  "-"
                }
              </div>
              ...
              <div className="lg:col-span-1 flex lg:block items-center">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <Link to={{ pathname: `/hizmet-detay/${e.WorkId}`, state: { prevTitle: "Tamamlanan Profesyonel Tahsilatları", prevPath: window.location.pathname, tabId: 6, queryPage: Number(readPageQueryString(window.location) ?? "1") } }}>
