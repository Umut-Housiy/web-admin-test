...
import { WaitingInvoiceProPaymentsInnerModel } from "../../Models";
...
export const ProWaitingInvoicePaymentActionList: FunctionComponent = () => {
  ...
  const getProWaitingInvoicePaymentActionList = async (order: number, searchText: string, page: number, take: number) => {

    const _result = await ApiService.getProWaitingInvoicePaymentActionList(page, take, searchText, order, parseInt(selectedFilterOption.key || "0"));
  ...
  const showCancelModal = (item) => {
    context.showModal({
      ...
      message: `${item.WorkId} hizmet numaralı işlemi iptal etmek istediğinize emin misiniz?`,
      onClick: async () => {
  ...
  const getDescription = (e: WaitingInvoiceProPaymentsInnerModel) => {
    ...
    if (e.WorkId) {
      return `#${e.WorkId} numaralı hizmet için ${returnPaymentType(e.PaymentType)}`
    }
  ...
  return (
    <div className="content-wrapper">
      <div className="portlet-wrapper">
        <h2 className="mb-5">
          Faturalandırılması Gereken Profesyonel İşlemleri
        </h2>
        <Table
          ref={tableEl}
          emptyListText={"İşlem Bulunamadı"}
          getDataFunction={getProWaitingInvoicePaymentActionList}
          isCustomFilter={true}
          noSearchBar={true}
          customFilter={
          ...
          addNewButton={
            <Button buttonMd textTiny color="text-blue-400" className="w-72 mr-3 px-3 my-3" design="button-blue-100" hasIcon icon={<PlusIcon className="icon-sm mr-2" />} text="Yeni Fatura Oluştur" onClick={() => history.push('/pro-yeni-fatura-olustur')} />
          }
          ...
          renderItem={(e: WaitingInvoiceProPaymentsInnerModel, i) => {
            return <div key={"list" + i} className="lg:grid-cols-11 px-2 border-b lg:h-20 border-gray-200 grid gap-4 items-center py-3 lg:py-0">
              ...
              <div className="lg:col-span-1 flex lg:block items-center">
                <span className="p-sm-gray-700 lg:hidden mr-2">Pro Bilgisi:</span>
              ...
              <div className="lg:col-span-2 flex lg:block items-center">
                ...
                <div className="flex items-center divide-x text-sm gap-x-3">
                  {(e.MustAddReceipt === true) ?
                    <Link to={`/pro-ekstre-odeme-kayitlari/${e.ProId}`} className="underline text-green-400 font-medium text-sm">
                  ...
                  {(e.AdvertisementIds || e.ReceiptId || e.WorkId) ? <></> :
