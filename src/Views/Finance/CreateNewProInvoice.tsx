...
import { ItemModel, ProModel, SellerModel, TotalPaymentType } from "../../Models"
...
export const CreateNewProInvoice: FunctionComponent = () => {
  ...
  const [selectedProIdList, setSelectedProIdList] = useState<number[]>([]);
  ...
  const createPaymentAction = async () => {
    ...
    const _result = await ApiService.createPaymentAction(Number(selectedInvoiceType.key), String(totalPrice), Number(selectedTaxValue.key), [], selectedProIdList)
    ...
    if (_result.succeeded === true) {
      context.showModal({
        ...
        onClose: () => {
          context.hideModal();
          history.push("/pro-faturalandirilmasi-gereken-islemler");
  ...
  return (
    <div className="content-wrapper">
      <div className="portlet-wrapper">
        <Link to="/pro-faturalandirilmasi-gereken-islemler" className="inline-block mb-5">
        ...
        <div className="mt-4 pt-4 border-t">
          <Tabs2ForProfessionals selectedProList={selectedProList} setSelectedProList={setSelectedProList} setSelectedProListToApi={setSelectedProListToApi} />
        </div>
