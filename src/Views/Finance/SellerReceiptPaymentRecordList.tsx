...
import { SellerPaymentsWithoutRecipteInnerModel } from "../../Models";
...
export const SellerReceiptPaymentRecordList: FunctionComponent = () => {
  ...
  const [allItems, setAllItems] = useState<SellerPaymentsWithoutRecipteInnerModel[]>([]);
  ...
  const getSellerPaymentActionsWithoutReceipt = async (order: number, searchText: string, page: number, take: number) => {

    const _result = await ApiService.getSellerPaymentActionsWithoutReceipt(Number(params.id), page, take, searchText, order);
  ...
  const createSellerReceipt = async () => {
    setProcessLoading(true);

    const _result = await ApiService.createSellerReceipt(Number(params.id), selectedPaymentIdList, plannedDate.getTime());
    ...
    if (_result.succeeded === true) {
      context.showModal({
        ...
        onClose: () => {
          ...
          history.push("/satici-ekstre-olustur")
        }
      });
    }
    else {
  ...
  return (
    <div className="content-wrapper">
      <div className="portlet-wrapper">
        <Link to={`${location?.state?.queryPage !== 1 ? `/satici-ekstre-olustur?sayfa=${location?.state?.queryPage}` : "/satici-ekstre-olustur"}`} className="inline-block mb-5">
          <div className="flex items-center text-sm text-gray-400 ">
            <ChevronRightIcon className="transform -rotate-180 icon-md mr-3" />
            Ekstre Oluştur
          </div>
        </Link>
        <h2 className="mb-4">Satıcı İşlem Detayı</h2>
        {selectedPaymentList.length > 0 &&
          <div className="py-3 mb-4 px-3 border rounded-lg bg-gray-100 flex items-center justify-between">
            <h5>Toplam Hakediş: <span className="ml-2 inline-block">
              <NumberView price={sumPrice} suffix={"TL"} />
            </span> </h5>
        ...
        <Table
          ...
          getDataFunction={getSellerPaymentActionsWithoutReceipt}
          ...
          renderItem={(e, i) => {
            return <div key={"list" + i} className={`${selectedPaymentList.find(i => i.PaymentId === e.PaymentId) ? "bg-blue-100" : ""} lg:grid-cols-8 px-2 border-b lg:h-20 mb-0.5 border-gray-200 grid gap-4 items-center py-3 lg:py-0`}>
              ...
              <div className="lg:col-span-1 flex lg:block items-center">
                ...
                <p className="p-sm underline font-medium">
                  #{e.UserOrderId}
                </p>              </div>
              <div className="lg:col-span-1 flex lg:block items-center">
      ...
      <Modal
        ...
        footer={
          <>
            <Button isLoading={processLoading} text="Ekstre Oluştur" design="button-blue-400 w-full mt-4" onClick={() => createSellerReceipt()} />
          </>
        }
      />
