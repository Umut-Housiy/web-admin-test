...
export const BlogEdit: FunctionComponent = () => {
  ...
  const getBlog = async () => {
    ...
    const _result = await ApiService.getBlog(Number(params.id ?? "0"));

    if (_result.succeeded === true) {
      setBlogTitle(_result.data.Title);
      setBlogDescription(_result.data.Description);
      setIsEnabled(_result.data.IsEnabled);
      setOldPreview(_result.data.PhotoUrl);
      setSelectedAuthorOption(authorOptions.find(x => x.key === String(_result.data.AuthorId)) ?? { key: "0", value: "Yazar" });
      setSelectedCategoryOption(categoryOptions.find(x => x.key === String(_result.data.CategoryId)) ?? { key: "0", value: "Kategori" });
      setSeoTitle(_result.data.SeoTitle ?? "");
      setSeoDescription(_result.data.SeoDescription ?? "")
      setLoading(false);
      setProcessLoading(false);
    }
    else {
      ...
      context.showModal({
        type: "Error",
        message: _result.message,
        onClose: () => { context.hideModal(); history.push("/blog-listesi"); }
      });
    }
  }
  ...
  const updateBlog = async () => {
    ...
    const _result = await ApiService.updateBlog(Number(params.id ?? "0"), blogTitle, Number(selectedAuthorOption.key ?? "0"), Number(selectedCategoryOption.key ?? "0"), blogDescription, isEnabled, selectedFile, seoTitle, seoDescription);
      ...
      <div className="portlet-wrapper">
        ...
        <div className="border-t border-gray-200">
          ...
          <div className="w-1/2">
            {
              loading ?
                <Loading inputSm />
                :
                <input className="form-input" type="text" placeholder="Blog Başlığı" value={blogTitle} onChange={(e) => { setBlogTitle(e.target.value); }} />
            }
          </div>
          ...
          <div className="w-1/2">
            {
              loading ?
                <Loading inputSm />
                :
                <FileUploader onFileSelected={item => { setSelectedFile(item) }} oldPreview={oldPreview} sizeDescription={"412x247 px"} />
            }
          </div>
          ...
          <div className="flex">
            ...
            <Button isLoading={processloading} textTiny className="w-72" design="button-blue-400" text="Kaydet ve Tamamla" onClick={() => { updateBlog(); }} />
          </div>
        </div>
      </div>
    </div>
