...
export const ProWorkOngoingList: FunctionComponent = () => {
  ...
  const getWorkOngoingList = async (order: number, searchText: string, page: number, take: number) => {

    const _result = await ApiService.getWorkOngoingList(page, take, searchText, order);
  ...
  return (
    <div className="content-wrapper">
      <div className="portlet-wrapper">
        <h2 className="mb-5">Devam Eden İşler</h2>
        <Table
          ref={tableEl}
          emptyListText={"İş Bulunamadı"}
          getDataFunction={getWorkOngoingList}
          ...
          renderItem={(e, i) => {
            return <div key={"list" + i} className="lg:grid-cols-7 px-2 border-b min-h-20 py-5 border-gray-200 hidden lg:grid flex gap-4 items-center">
              ...
              <div className="lg:col-span-1 flex items-center gap-1 justify-between">
                ...
                <div>
                  <Link to={{ pathname: `/hizmet-detay/${e.WorkId}`, state: { prevTitle: "Devam Eden İşler", prevPath: window.location.pathname, tabId: 1, queryPage: Number(readPageQueryString(window.location) ?? "1") } }}>
                    <ChevronRightIcon className="w-8 h-5 hover:hover:text-blue-400 text-gray-700 cursor-pointer transition-all" onClick={() => { }} />
                  </Link>
