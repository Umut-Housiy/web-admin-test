...
export const CampaignList: FunctionComponent<CampaignListPropModel> = (props: CampaignListPropModel) => {
  ...
  const getProductCampaignList = async (order: number, searchText: string, page: number, take: number) => {

    const _result = await ApiService.getProductCampaignList(props.productId, page, take, searchText, order);
  ...
  return (
    <>
      <Table
        ...
        getDataFunction={getProductCampaignList}
        header={
          ...
          return (
            <div key={"list" + i} className="lg:grid-cols-9 px-2 border-b  border-gray-200 grid gap-4 items-center py-6 ">
              ...
              <div className="lg:col-span-1 flex gap-2 text-sm font-medium text-yellow-600  items-center">
                <p className="p-sm">
                  {e.CampaignTypeByte === 1 ?
                    "Tekil Ürün Kampanyası"
