...
export interface VariationListPropModel {
  productId: number
}

export const VariationList: FunctionComponent<VariationListPropModel> = (props: VariationListPropModel) => {
  ...
  const getProductsOtherVariations = async (order: number, searchText: string, page: number, take: number) => {

    const _result = await ApiService.getProductsOtherVariations(props.productId, page, take, searchText, order);
  ...
  return (
    <>
      <Table
        ...
        getDataFunction={getProductsOtherVariations}
        header={
          <div className=" lg:grid-cols-7 px-2 border-b border-t lg:h-14 border-gray-200 hidden lg:grid gap-4 flex items-center">
        ...
        renderItem={(e: ProductListInnerModel, i) => {
          return (
            <div key={"list" + i} className="lg:grid-cols-7 px-2 border-b  border-gray-200 grid gap-4 items-center py-6 ">
