...
export const SeoProductCategory: FunctionComponent = () => {
  ...
  const getSellerCategorySeoData = async (Id: number) => {
    setLoading(true);
    setShowSeoModal(true);

    const _result = await ApiService.getSellerCategorySeoData(Id);
  ...
  const updateSellerCategorySeoData = async () => {
    setProcessLoading(true);

    const _result = await ApiService.updateSellerCategorySeoData(modalId, title, description);
  ...
  return (
    <div className="content-wrapper">
      <div className="portlet-wrapper">
        <h2 className="pb-5">Ürün Kategori SEO</h2>
        <Table
          ref={tableEl}
            ...
            return <div className="lg:grid-cols-12 px-2 border-b min-h-20 py-5 border-gray-200 grid gap-4 items-center" key={i} >
              <div className="lg:col-span-12 flex justify-between items-center">
                ...
                <div className="text-gray-700 flex gap-2 items-center">
                  <EditIcon className="icon-sm hover:text-blue-400 cursor-pointer transition-all" onClick={() => { getSellerCategorySeoData(e.Id); }} />
                </div>
              </div>
            </div>
          }}
      ...
      <Modal
        ...
        body=
        {
          <>
            ...
            <Button isLoading={processLoading || loading} textTiny className="w-full mt-4 " design="button-blue-400" text="Kaydet" onClick={() => { updateSellerCategorySeoData(); }} />
          </>
        }
      />
    </div>
