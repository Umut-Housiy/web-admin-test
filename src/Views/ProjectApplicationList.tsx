...
export const ProjectApplicationList: FunctionComponent = () => {
  ...
  const getProjectList = async (order: number, searchText: string, page: number, take: number) => {
    ...
    const _result = await ApiService.getProjectList(page, take, searchText, order, 1);
  ...
  return (
    <div className="content-wrapper">
      <div className="portlet-wrapper">
        <h2 className="mb-5">Onay Bekleyen Proje Listesi</h2>
        <Table
          ref={tableEl}
          ...
          renderItem={(e, i) => {
            return <div key={"list" + i} className="lg:grid-cols-6 px-2 border-b h-20 border-gray-200 hidden lg:grid flex gap-4 items-center">
              ...
              <div className="lg:col-span-1 flex justify-between">
                <div className="rounded-full w-36 py-1 bg-red-100 text-yellow-600 text-sm text-center my-auto">Onay Bekliyor</div>
                <div className="text-gray-700 flex gap-2 items-center">
                  <Link to={{ pathname: `${"/proje-detay/" + e.Id}`, state: { prevTitle: "Onay Bekleyen Projeler", prevPath: window.location.pathname, tabId: 1, IsIdeaApproved: false, queryPage: Number(readPageQueryString(window.location) ?? "1") } }} >
