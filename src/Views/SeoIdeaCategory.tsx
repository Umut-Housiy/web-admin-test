...
export const SeoIdeaCategory: FunctionComponent = () => {
  ...
  const getIdeaCategorySeoData = async (Id: number) => {
    setLoading(true);
    setShowSeoModal(true);

    const _result = await ApiService.getIdeaCategorySeoData(Id);
  ...
  const updateIdeaCategorySeoData = async () => {
    setProcessLoading(true);

    const _result = await ApiService.updateIdeaCategorySeoData(modalId, title, description);
  ...
  return (
    <div className="content-wrapper">
      <div className="portlet-wrapper">
        <h2 className="pb-5">Fikir Kategori SEO</h2>
        <Table
          ref={tableEl}
            ...
            return <div className="lg:grid-cols-12 px-2 border-b min-h-20 py-5 border-gray-200 grid gap-4 items-center" key={i} >
              <div className="lg:col-span-12 flex justify-between items-center">
                ...
                <div className="text-gray-700 flex gap-2 items-center">
                  <EditIcon className="icon-sm hover:text-blue-400 cursor-pointer transition-all" onClick={() => { getIdeaCategorySeoData(e.Id); }} />
                </div>
              </div>
            </div>
          }}
      ...
      <Modal
        ...
        footer={
          <Button isLoading={processLoading || loading} textTiny className="w-full mt-4 " design="button-blue-400" text="Kaydet" onClick={() => { updateIdeaCategorySeoData(); }} />
        }
      />
    </div>
  )
