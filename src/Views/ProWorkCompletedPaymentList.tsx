...
export const ProWorkCompletedPaymentList: FunctionComponent = () => {
  ...
  const getWorkCompletedPaymentList = async (order: number, searchText: string, page: number, take: number) => {

    const _result = await ApiService.getWorkCompletedPaymentList(page, take, searchText, order);
  ...
  return (
    <div className="content-wrapper">
      <div className="portlet-wrapper">
        <h2 className="mb-5">Tamamlanan Ödemeler</h2>
        <Table
          ref={tableEl}
          emptyListText={"Ödeme Bulunamadı"}
          getDataFunction={getWorkCompletedPaymentList}
            ...
            return <div key={"list" + i} className="lg:grid-cols-7 px-2 border-b min-h-20 py-5 border-gray-200 hidden lg:grid flex gap-4 items-center">
              ...
              <div className="lg:col-span-1 flex items-center  justify-between">
                <p className="font-medium text-sm">
                  <div className="w-36 text-center py-1 rounded-full bg-green-100 text-green-400">Tahsil Edildi</div>
                </p>
                <div>
                  <Link to={{ pathname: `/is-odeme-detay/${e.WorkId}`, state: { prevTitle: "Tamamlanan Ödemeler", prevPath: window.location.pathname, queryPage: Number(readPageQueryString(window.location) ?? "1") } }}>
