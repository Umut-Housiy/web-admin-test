...
export const AdminUserAdd: FunctionComponent = () => {
  ...
  const [permissionList, setPermissionList] = useState<string[]>(["Home"]);
  ...
  const [selectedUserType, setSelectedUserType] = useState<{ key: string, value: string }>({ key: "0", value: "Kullanıcı Rolü" });
  ...
  const handleCreateAdminUser = async () => {
    ...
    const _result = await ApiService.createAdmin(userName, email, password, permissionList, isSuper, Number(selectedUserType.key));
  ...
  return (
    <div className="content-wrapper">
      <div className="portlet-wrapper">
        <h2 className="mb-5">Yeni Kullanıcı Oluştur</h2>
        ...
        <div className="grid grid-cols-2">
          <div className="lg:col-span-1">
            ...
            <Dropdown
              isDropDownOpen={currentOpenedFilterButton === "userType"}
              onClick={() => { setCurrentOpenedFilterButton("userType"); }}
        ...
        <div className="w-full flex">
          <Button isLoading={processLoading} textTiny className="w-72 ml-auto" design="button-blue-400" text="Kaydet ve Tamamla" onClick={() => { handleCreateAdminUser(); }} />
        </div>
      </div>
    </div>
  )
