...
export const ProPanelSliderList: FunctionComponent = () => {
  ...
  const getSliderList = async (order: number, searchText: string, page: number, take: number) => {
    ...
    const _result = await ApiService.getSliderList(page, take, searchText, order, 4);
  ...
  return (
    <div className="content-wrapper">
      <div className="portlet-wrapper">
        <h2 className="pb-5 border-b">Pro Panel Slider Yönetimi</h2>
        <Table
          ref={tableEl}
          ...
          addNewButton={
            <Button buttonMd textTiny color="text-blue-400" className="w-72" design="button-blue-100" text="Yeni Slider Oluştur" hasIcon icon={<PlusIcon className="icon-sm mr-2" />}
              onClick={() => { history.push(`/slider-ekle/4`) }}
            />}
